...
ALL_SOURCES = SOURCE_CLASSES

DEFAULT_SOURCE = MangaDexSource.id

__all__ = [
    "BASE_HEADERS", "DEFAULT_SOURCE", "DEFAULT_UA", "SOURCES", "SOURCE_CLASSES", "ALL_SOURCES",
...
def load_custom_sources():
    """Discover and dynamically load modular Python scraper plugins.

    Scans:
    1. mangasurf/sources/customsources/*.py
    2. ~/.mangasurf/sources/*.py
    """
    ...
    search_dirs = [
        os.path.join(os.path.dirname(__file__), "customsources"),
        os.path.expanduser("~/.mangasurf/sources"),
    ]
    ...
    for s_dir in search_dirs:
        ...
        for fname in sorted(os.listdir(s_dir)):
            if fname.endswith(".py") and not fname.startswith("__"):
                ...
                try:
                    mod_name = f"custom_source_{os.path.splitext(fname)[0]}"
                    ...
                    if spec and spec.loader:
                        ...
                        for attr_name in dir(mod):
                            ...
                            if (
                                ...
                                if attr not in SOURCE_CLASSES:
                                    SOURCE_CLASSES.append(attr)
                                    SOURCES[attr.id] = attr
                                    logger.info("Loaded custom scraper plugin: %s (%s)", attr.name, attr.id)
                except Exception as e:
                    logger.warning("Failed to load custom source plugin from %s: %s", fpath, e)


def reload_sources():
    """Reload and refresh the sources registry dynamically."""
    load_custom_sources()
    return list_sources()
