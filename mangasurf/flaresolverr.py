...
def is_curl_cffi_available() -> dict:
    try:
        import curl_cffi
        return {
            "available": True,
            "version": getattr(curl_cffi, "__version__", "0.16.1"),
            "fingerprint": "Chrome 124 (JA3/JA4 TLS Impersonation)",
            "speed": "Sub-millisecond native C-binding",
        }
    except Exception as e:
        return {
            "available": False,
...
def test_curl_cffi_ping(target_url: str = "https://cloudflare.com") -> dict:
    try:
        ...
        return {"ok": True, "status": r.status_code, "ping_ms": ms, "url": target_url}
    except Exception as e:
