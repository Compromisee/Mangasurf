...
function getDownloadedMeta(url, title) {
    if (!libraryCache || !libraryCache.length) return null
    const normUrl = (url || '').trim().toLowerCase()
    const normTitle = (title || '').trim().toLowerCase()
    const match = libraryCache.find(b => {
        if (b.url && b.url.toLowerCase() === normUrl) return true
        if (b.title && b.title.toLowerCase() === normTitle) return true
        return false
    })
                ...
                ${isDownloaded ? `
                  ...
                  <div class="thumb-hover-overlay">
                    <div class="dl-hover-content">
                      <div class="dl-hover-icon"><span class="mi">check_circle</span></div>
                      <div class="dl-hover-title">DOWNLOADED</div>
                      <div class="dl-hover-fraction">${dlCount} ${dlCount === 1 ? 'Chapter' : 'Chapters'} Downloaded</div>
                      <div class="dl-hover-saved">Saved in Library</div>
                    </div>
                  </div>
                ` : ''}

                <button class="card-queue-btn" data-manga-url="${esc(r.url)}" data-source="${esc(r.source || '')}" data-title="${esc(r.title)}" data-cover="${esc(r.cover || '')}" title="Add to Download Queue" type="button" aria-label="Add to Queue">
                  <span class="mi">playlist_add</span>
                </button>
              </div>
              <div class="meta">
...
function extractChapterNumber(str) {
    ...
    const s = str.trim()
    // Skip full date strings like 2026-08-18
    if (/^\d{4}[-/]\d{2}[-/]\d{2}/.test(s)) return null
    // Match standard chapter patterns: "Chapter 12", "Ch. 12.5", "Ep. 5", "#12"
    const match = s.match(/(?:chapter|ch|ep|episode|volume|vol|#)\s*\.?\s*(\d+(?:\.\d+)?)/i)
    if (match && match[1]) {
        return parseFloat(match[1])
    }
    // Match leading numbers e.g. "12 - Title" or "12.5"
    const leadMatch = s.match(/^(\d+(?:\.\d+)?)(?:\s*[-–:]|\s+|$)/)
    if (leadMatch && leadMatch[1]) {
        return parseFloat(leadMatch[1])
    }
    return null
}
...
function chapterNumber(chapter) {
    ...
    if (typeof chapter.num === 'number') return chapter.num
    if (typeof chapter.number === 'number') return chapter.number
    if (typeof chapter.chapter === 'number') return chapter.chapter
    
    const raw = chapter.number ?? chapter.num ?? chapter.chapter ?? chapter.name ?? chapter.title ?? ''
    return extractChapterNumber(String(raw))
}
...
async function openDetail(url, source) {
    ...
    const match = libraryCache.find(b => {
        ...
        const iNorm = (info.title || '').toLowerCase().replace(/[^a-z0-9]/g, '')
        if (bNorm && iNorm && bNorm === iNorm) return true
        return false
    })
    
    detail.haveNums = new Set()
    detail.haveNames = new Set()
    detail.have = new Set()

    function addDownloadedChapter(raw) {
        if (!raw) return
        const str = String(raw).trim()
        if (!str) return
        detail.haveNames.add(str.toLowerCase())
        detail.have.add(str)
        const val = extractChapterNumber(str)
        if (val !== null && !Number.isNaN(val)) detail.haveNums.add(val)
    }

    if (match) {
        if (Array.isArray(match.items)) {
            match.items.forEach(it => {
                if (it.label) addDownloadedChapter(it.label)
                if (it.path) {
                    const fname = it.path.split(/[\/\\]/).pop().replace(/\.[^.]+$/, '')
                    addDownloadedChapter(fname)
                }
            })
        }
        if (match.chapters && typeof match.chapters === 'object') {
            Object.keys(match.chapters).forEach(k => addDownloadedChapter(k))
        }
        if (Array.isArray(match.downloaded_chapters)) {
            match.downloaded_chapters.forEach(c => addDownloadedChapter(c.name || c))
        }
    }

    try {
        const lib = await call('downloaded_status', url)
        const statusChapters = lib?.chapters || lib?.downloaded || lib?.status?.[url]?.downloaded || lib?.status?.[url]?.chapter_names || []
        if (Array.isArray(statusChapters)) {
            statusChapters.forEach(c => addDownloadedChapter(c))
        }
    } catch {}
...
function getMatchingDownloadedCount() {
    if (!detail.chapters || !detail.chapters.length) {
        return detail.haveNums ? detail.haveNums.size : (detail.have ? detail.have.size : 0)
    }
    let count = 0
    detail.chapters.forEach((ch, idx) => {
        if (isChapterDownloaded(ch, idx)) count++
    })
    return count
}
...
function renderDetail(info) {
    ...
    const haveCount = getMatchingDownloadedCount()
    const tags = []
    if (haveCount > 0) tags.push([`${haveCount} Downloaded`, 'status-tag dl-tag', 'downloaded'])
...
function isChapterDownloaded(chapter, index) {
    if (!detail.haveNums || !detail.haveNames) return false
    if (detail.haveNums.size === 0 && detail.haveNames.size === 0) return false

    // 1. Exact numeric chapter match
    const chNum = chapterNumber(chapter)
    if (chNum !== null && detail.haveNums.has(chNum)) {
        return true
    }

    // 2. Exact normalized name match
    const name = String(chapter?.name || chapter?.title || chapter?.chapter || `Chapter ${index + 1}`).trim().toLowerCase()
    if (name && detail.haveNames.has(name)) {
        return true
    }

    // 3. Fallback: exact 1-based index if no number was extracted from label
    if (chNum === null && detail.haveNums.has(index + 1)) {
        return true
    }
...
function renderChapters() {
    ...
    detail.chapters.forEach((ch, idx) => {
        if (isChapterDownloaded(ch, idx)) downloadedCount++
    })
    if (!downloadedCount && detail.have && detail.have.size) {
        downloadedCount = detail.have.size
    }

    if (have) {
        have.hidden = downloadedCount === 0
        have.textContent = `${downloadedCount} Downloaded`
    }
...
function updateDownloadLabel() {
    ...
    const readBtn = $('#d-read')
    if (readBtn) {
        readBtn.hidden = false
        const hasLocal = detail.have && detail.have.size > 0
        readBtn.innerHTML = hasLocal
            ? '<span class="mi">menu_book</span>Read (Offline / Local)'
            : '<span class="mi">auto_stories</span>Read Online (Instant Stream)'
    }
}
...
function pickChapters(which) {
    ...
    if (which === 'all') for (const { index } of rows) detail.selected.add(index)
        ...
        for (const { chapter, index } of rows) {
            if (!isChapterDownloaded(chapter, index)) detail.selected.add(index)
        }
...
function wireDetail() {
    ...
    $('#d-chapters').addEventListener('click', async e => {
        const readBtn = e.target.closest('.ch-read-online-btn')
        if (readBtn) {
            e.stopPropagation()
            e.preventDefault()
            ...
            if (chapter) {
                const chUrl = chapter.url || readBtn.dataset.chapterUrl
                if (chUrl) {
                    closeDetail()
    ...
    $('#d-read').addEventListener('click', async () => {
        // 1. Check if local downloaded files exist on disk
        const lib = await call('get_library_entry', detail.url)
        const entry = lib?.entry || lib
        const firstLocal = (entry?.items || [])[0]
        if (firstLocal?.path) {
            closeDetail()
            openPath(firstLocal.path)
            return
        }
        ...
        if (targetChapter) {
            const chUrl = targetChapter.url || targetChapter.link
            if (chUrl) {
                closeDetail()
        ...
        toast('No chapter available to stream online')
    })
}
...
async function openPath(path) {
    ...
    if (res.kind === 'pages') {
        ...
        try {
            ...
            if (book.pageNames && book.pageNames.length) {
                res.names = book.pageNames
            }
...
async function loadChapters() {
    ...
    let dir = ''
    if (state.book.path) {
        dir = state.book.path.replace(/[\\/][^\\/]+$/, '')
    }
    const res = await call('reader_chapters', state.book.url || '', dir)
    ...
    if (state.chapters.length > 1) {
        list.innerHTML = state.chapters.map(c => `
            <div class="chap ${c.path === state.book.path ? 'on' : ''}" data-open="${esc(c.path)}">
              ${esc(c.label)}
              <span class="cs">${c.pages ? `${c.pages} pages` : ''}${c.position ? ` · ${Math.round((c.position.fraction || 0) * 100)}%` : ''}</span>
            </div>`).join('')
        return
    }

    // Multi-chapter bundle CBZ with internal chapter sections
    if (state.book.toc && state.book.toc.length > 1) {
        list.innerHTML = state.book.toc.map(t => `
            <div class="chap" data-goto-toc="${esc(t.href)}">
              ${esc(t.label)}
              <span class="cs">Chapter Section</span>
            </div>`).join('')
        return
    }

    list.innerHTML = '<p class="empty">No sibling chapters.</p>'
}
...
function wire() {
    ...
    document.addEventListener('click', async e => {
        const queueBtn = e.target.closest('.card-queue-btn')
        if (queueBtn) {
            e.stopPropagation()
            e.preventDefault()
            const mangaUrl = queueBtn.dataset.mangaUrl
            const mangaSrc = queueBtn.dataset.source
            const title = queueBtn.dataset.title || 'Manga'
            const cover = queueBtn.dataset.cover || ''

            queueBtn.disabled = true
            queueBtn.classList.add('loading')
            queueBtn.innerHTML = '<span class="mi spin-icon">sync</span>'

            try {
                toast(`Adding "${title}" to Download Queue…`)
                const res = await call('add_to_cart', {
                    url: mangaUrl,
                    title: title,
                    cover: cover,
                    source: mangaSrc
                ...
                if (res?.ok !== false) {
                    ...
                    queueBtn.innerHTML = '<span class="mi">check</span>'
                    queueBtn.title = 'Added to queue'
                    showDownloadToast(`Enqueued: ${title}`, 'Added all chapters to download queue', cover)
                    toast(`Added "${title}" to download queue`)
                    refreshQueue()
                } else {
                    queueBtn.classList.remove('loading')
                    queueBtn.disabled = false
                    queueBtn.innerHTML = '<span class="mi">playlist_add</span>'
                    toast(`Queue error: ${res?.error || 'Could not add'}`)
                }
            } catch (err) {
                queueBtn.classList.remove('loading')
                queueBtn.disabled = false
                queueBtn.innerHTML = '<span class="mi">playlist_add</span>'
                toast(`Could not add to queue: ${err.message || err}`)
            }
            return
        }
    ...
    $('#btn-tools-rescan')?.addEventListener('click', async () => {
        const btn = $('#btn-tools-rescan')
        const msg = $('#tools-status-msg')
        btn.disabled = true
        btn.innerHTML = '<span class="mi spin-icon">sync</span>Scanning…'
        try {
            const res = await call('rescan_library_books')
            toast(`Rescanned library: ${res?.total || 0} series indexed`)
            if (msg) {
                msg.style.display = 'block'
                msg.textContent = `✔ Rescan complete: ${res?.total || 0} series indexed on disk`
            }
            await refreshLibrary()
        } catch (e) {
            ...
            btn.disabled = false
            btn.innerHTML = '<span class="mi">sync</span>Rescan All Books'
        }
    })

    $('#btn-tools-fix-covers')?.addEventListener('click', async () => {
        const btn = $('#btn-tools-fix-covers')
        const msg = $('#tools-status-msg')
        btn.disabled = true
        btn.innerHTML = '<span class="mi spin-icon">sync</span>Fixing…'
        try {
            const res = await call('fix_missing_covers')
            toast(`Cover check: ${res?.fixed || 0} covers restored`)
            if (msg) {
                msg.style.display = 'block'
                msg.textContent = `✔ Checked covers: ${res?.fixed || 0} covers updated`
            }
            await refreshLibrary()
        } catch (e) {
            toast(`Fix covers failed: ${e.message}`)
        } finally {
            btn.disabled = false
            btn.innerHTML = '<span class="mi">image</span>Fix Missing Covers'
        }
    })

    $('#btn-tools-rebuild-meta')?.addEventListener('click', async () => {
        const btn = $('#btn-tools-rebuild-meta')
        const msg = $('#tools-status-msg')
        btn.disabled = true
        btn.innerHTML = '<span class="mi spin-icon">sync</span>Rebuilding…'
        try {
            const res = await call('rebuild_library_metadata')
            toast(`Rebuilt ComicInfo.xml & manga.json for ${res?.rebuilt || 0} series`)
            if (msg) {
                msg.style.display = 'block'
                msg.textContent = `✔ Rebuilt XML & JSON metadata for ${res?.rebuilt || 0} series folders`
            }
            await refreshLibrary()
        } catch (e) {
            toast(`Metadata rebuild failed: ${e.message}`)
        } finally {
            btn.disabled = false
            btn.innerHTML = '<span class="mi">description</span>Rebuild XML & JSON'
        }
    })

    $('#btn-tools-clean-orphans')?.addEventListener('click', async () => {
        const btn = $('#btn-tools-clean-orphans')
        const msg = $('#tools-status-msg')
        btn.disabled = true
        btn.innerHTML = '<span class="mi spin-icon">sync</span>Cleaning…'
        try {
            const res = await call('clean_library_cache')
            toast(`Cleaned ${res?.cleaned || 0} temporary cache files`)
            if (msg) {
                msg.style.display = 'block'
                msg.textContent = `✔ Cleaned ${res?.cleaned || 0} temporary cache files`
            }
        } catch (e) {
            toast(`Clean failed: ${e.message}`)
        } finally {
            btn.disabled = false
            btn.innerHTML = '<span class="mi">cleaning_services</span>Clean Cache'
        }
    })

    // ---- database & declutter settings
    $('#set-db-mal')?.addEventListener('change', e =>
        pushSettings({ db_mal_enabled: e.target.checked }))
    $('#set-db-pornhwadb')?.addEventListener('change', e =>
        pushSettings({ db_pornhwadb_enabled: e.target.checked }))
    $('#set-db-mangabaka')?.addEventListener('change', e =>
        pushSettings({ db_mangabaka_enabled: e.target.checked }))
    $('#set-db-crossref')?.addEventListener('change', e =>
        pushSettings({ db_crossref_enabled: e.target.checked }))
    $('#set-declutter-titles')?.addEventListener('change', e =>
        pushSettings({ declutter_titles: e.target.checked }))
    $('#set-prefer-curlcffi')?.addEventListener('change', e =>
        pushSettings({ prefer_curl_cffi: e.target.checked }))

    // ---- curl_cffi, plugins & user data backup handlers
    $('#btn-curlcffi-test')?.addEventListener('click', async () => {
        const btn = $('#btn-curlcffi-test')
        btn.disabled = true
        btn.innerHTML = '<span class="mi spin-icon">sync</span>Testing…'
        try {
            const res = await call('curl_cffi_test')
            if (res?.ok) {
                toast(`curl_cffi TLS Ping: HTTP ${res.status} in ${res.ping_ms}ms`)
            } else {
                toast(`curl_cffi ping failed: ${res?.error || 'Unavailable'}`)
            }
        } catch (e) {
            toast(`curl_cffi error: ${e.message}`)
        } finally {
            btn.disabled = false
            btn.innerHTML = '<span class="mi">network_check</span>Test curl_cffi Ping'
        }
    })

    $('#btn-reload-sources')?.addEventListener('click', async () => {
        const btn = $('#btn-reload-sources')
        btn.disabled = true
        btn.innerHTML = '<span class="mi spin-icon">sync</span>Reloading…'
        try {
            const res = await call('reload_scraper_plugins')
            toast(`Reloaded plugins: ${res?.count || 0} active sources`)
            await fillSources()
            refreshSources()
        } catch (e) {
            toast(`Failed to reload sources: ${e.message}`)
        } finally {
            btn.disabled = false
            btn.innerHTML = '<span class="mi">extension</span>Reload Plugins &amp; Sources'
        }
    })

    $('#btn-export-userdata')?.addEventListener('click', async () => {
        const res = await call('export_user_data')
        if (res?.ok && res?.data) {
            const blob = new Blob([JSON.stringify(res.data, null, 2)], { type: 'application/json' })
            const url = URL.createObjectURL(blob)
            const a = document.createElement('a')
            a.href = url
            a.download = `mangasurf-userdata-${new Date().toISOString().slice(0, 10)}.json`
            a.click()
    ...
    $('#btn-import-userdata')?.addEventListener('click', async () => {
        ...
        input.onchange = async () => {
            ...
            try {
                ...
                const res = await call('import_user_data', data)
                if (res?.ok) {
                    toast(`Imported ${res.imported_sections || 'all'} data sections!`)
                    await refreshLibrary()
                    await refreshLibraryFolders()
                    await refreshStats()
                } else {
                    toast(`Import failed: ${res?.error || 'Invalid file'}`)
                }
            } catch (e) {
    ...
    $('#btn-clear-search-history')?.addEventListener('click', async () => {
        if (!confirm('Clear all search history queries?')) return
        await call('clear_user_history', 'search')
        toast('Search history cleared')
    })

    $('#btn-clear-suggestions-cache')?.addEventListener('click', async () => {
        if (!confirm('Clear offline metadata suggestions cache?')) return
        await call('clear_user_history', 'suggestions')
        toast('Suggestions cache cleared')
    })
    ...
    $('#chap-items')?.addEventListener('click', e => {
        ...
        if (tocEl) {
            ...
            const mv = $('#mv')
            if (mv && href) {
                const target = mv.book?.resolveHref?.(href)
                if (target && typeof target.index === 'number') {
                    mv.goTo(target.index)
                    $('#r-chaplist').hidden = true
                }
            }
        }
...
async function boot() {
    ...
    const dbMalCheck = $('#set-db-mal')
    if (dbMalCheck) dbMalCheck.checked = s.db_mal_enabled !== false
    const dbPdbCheck = $('#set-db-pornhwadb')
    if (dbPdbCheck) dbPdbCheck.checked = s.db_pornhwadb_enabled !== false
    const dbMbCheck = $('#set-db-mangabaka')
    if (dbMbCheck) dbMbCheck.checked = s.db_mangabaka_enabled !== false
    const dbCrCheck = $('#set-db-crossref')
    if (dbCrCheck) dbCrCheck.checked = s.db_crossref_enabled !== false
    const declutterCheck = $('#set-declutter-titles')
    if (declutterCheck) declutterCheck.checked = s.declutter_titles !== false
