...
def publication_entry(base, row):
    ...
    try:
        from .config import load_settings
        from .metadata import declutter_title
        s = load_settings()
        raw_title = row.get("title") or "Untitled"
        clean_title = declutter_title(raw_title) if s.get("declutter_titles", True) else raw_title
    except Exception:
        clean_title = row.get("title") or "Untitled"

    parts = [
        f"    {element('title', clean_title)}",
        f"    {element('id', row['id'])}",
