...
The scraping logic that used to live here now lives in ``mangasurf.sources``,
one module per site. This file remains so older imports keep working:

    from mangasurf.scraper import WeebCentralScraper   # still fine

New code should use the registry instead::

    from mangasurf.sources import get_source, source_for_url
