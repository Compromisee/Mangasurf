...
import sys
import os
# Allow running this file directly (python mangasurf/packager.py, or an IDE's
...
def create_cbz(chapter_dirs, out_path, metadata: dict = None):
    ...
    so readers keep the correct page order. Embeds ComicInfo.xml in archive and folder.
    """
    ...
    with zipfile.ZipFile(out_path, "w", zipfile.ZIP_STORED) as zf:
        ...
        # Embed ComicInfo.xml metadata inside CBZ and series directory
        meta = metadata or {}
        if not meta and pairs:
            meta = {"title": os.path.splitext(os.path.basename(out_path))[0]}
        try:
            from .metadata import generate_comic_info_xml, write_folder_metadata
            xml_str = generate_comic_info_xml(meta)
            zf.writestr("ComicInfo.xml", xml_str)
            out_dir = os.path.dirname(out_path)
            write_folder_metadata(out_dir, meta)
        except Exception:
            pass
...
PACKAGERS = {
    "cbz": lambda dirs, path, title: create_cbz(dirs, path, metadata={"title": title}),
