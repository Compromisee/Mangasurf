...
class DownloadEngine:
    ...
    def run(self) -> dict:
        ...
        # Write metadata JSON and ComicInfo.xml into the series folder
        try:
            from .metadata import write_folder_metadata
            dl_chapters_list = []
            for c in selected:
                c_name = c["name"]
                dl_chapters_list.append({
                    "name": c_name,
                    "pages": done_pages.get(c_name, 0),
            ...
            meta = {
                ...
                "genres": info.get("tags") or [],
                "format": opt.format,
                "cover": info.get("cover") or "",
                "total_chapters": completed,
                "downloaded_chapters": dl_chapters_list,
                "chapters": dl_chapters_list,
                "total_pages": total_pages,
                "size_bytes": byte_total,
                "outputs": outputs,
                "reading_progress": {"last_read_chapter": None, "fraction": 0.0},
                "downloaded_at": time.strftime("%Y-%m-%d %H:%M:%S"),
            }
            write_folder_metadata(manga_dir, meta)
        except Exception:
            logger.debug("Failed to write series metadata", exc_info=True)
