...
class Api(ReaderApi, metaclass=_SafeApiMeta):
    ...
    def export_user_data(self):
        """Export comprehensive user data (history, suggestions, library, folders, stats, settings)."""
        ...
        try:
            payload = {
                ...
                "bookmark_folders": features.get_bookmark_folders(),
                "library_folders": load_settings().get("library_folders", []),
                "search_history": features.get_history(100),
    ...
    def import_user_data(self, payload: dict = None):
        """Import and merge user data bundle."""
        ...
        try:
            ...
            if "library" in payload and isinstance(payload["library"], dict):
                cur_lib = library.load_library()
                cur_lib.update(payload["library"])
                library.save_library(cur_lib)
                imported_count += 1
            if "bookmarks" in payload and isinstance(payload["bookmarks"], list):
                for b in payload["bookmarks"]:
                    if isinstance(b, dict) and b.get("url"):
                        library.add_bookmark(b)
                imported_count += 1
            if "library_folders" in payload and isinstance(payload["library_folders"], list):
                s = load_settings()
                cur = s.get("library_folders") or []
                for f in payload["library_folders"]:
                    if f and f not in cur:
                        cur.append(f)
                update_settings({"library_folders": cur})
                imported_count += 1
            if "search_history" in payload and isinstance(payload["search_history"], list):
                for h in payload["search_history"]:
                    if isinstance(h, dict) and h.get("query"):
                        features.add_history(h["query"], h.get("source", "all"), h.get("count", 0))
                imported_count += 1

            return {"ok": True, "imported_sections": imported_count}
        except Exception as e:
            return {"ok": False, "error": str(e)}

    def clear_user_history(self, category: str = "search"):
        """Selectively clear user history or cache."""
        from .. import features
        try:
            if category == "search":
                features.clear_history()
                ...
                from ..paths import data_dir
                cache_path = os.path.join(data_dir(), "metadata_cache.json")
    ...
    def curl_cffi_status(self):
        """Check status and capabilities of native curl_cffi TLS impersonation."""
        from ..flaresolverr import is_curl_cffi_available
        return {"ok": True, **is_curl_cffi_available()}

    def curl_cffi_test(self, target_url: str = None):
        """Ping a live Cloudflare-protected endpoint via curl_cffi."""
        from ..flaresolverr import test_curl_cffi_ping
        return test_curl_cffi_ping(target_url or "https://cloudflare.com")

    def reload_scraper_plugins(self):
        """Hot-reload custom sources from ~/.mangasurf/sources/."""
        from ..sources import reload_sources
    ...
    def smart_covers(self, root: str = None, overwrite: bool = False,
        ...
        candidate by the rules in :func:`mangasurf.covers.auto_pick` -- exact
    ...
    def downloaded_status(self, items: list = None):
        """Download status for a batch of search results or single series URL."""
        if isinstance(items, str):
            items = [{"url": items}]
        rows = [i for i in (items or []) if isinstance(i, dict) and i.get("url")]
        if not rows:
            return {"ok": True, "status": {}, "chapters": [], "downloaded": []}

        # Read the library once for the whole batch.
        lib = library.load_library()
        lib_entries = list(lib.values())
        settings = load_settings()
        out = {}
        for item in rows:
            url = item.get("url")
            ...
            # 2. Cross-reference across sources by title and synonyms!
            if not entry and settings.get("db_crossref_enabled", True):
                try:
                    from .metadata import cross_reference_downloaded
                    entry = cross_reference_downloaded(item, lib_entries)
                except Exception:
                    pass
            ...
            chapters_dict = entry.get("chapters", {}) or {}
            ch_names_set = set(chapters_dict.keys())

            # Also collect from outputs on disk
            for out_p in entry.get("outputs") or []:
                if out_p:
                    fname = os.path.splitext(os.path.basename(out_p))[0]
                    ch_names_set.add(fname)

            # Also scan series directory on disk
            dir_path = entry.get("directory")
            if dir_path and os.path.isdir(dir_path):
                try:
                    for f in os.listdir(dir_path):
                        ext = os.path.splitext(f)[1].lower()
                        if ext in (".cbz", ".cbr", ".epub", ".pdf", ".zip"):
                            ch_names_set.add(os.path.splitext(f)[0])
                    # Check manga.json
                    meta_p = os.path.join(dir_path, "manga.json")
                    if os.path.isfile(meta_p):
                        with open(meta_p, "r", encoding="utf-8") as mfp:
                            m_meta = json.load(mfp)
                            for c in m_meta.get("downloaded_chapters") or m_meta.get("chapters") or []:
                                c_name = c.get("name") if isinstance(c, dict) else str(c)
                                if c_name:
                                    ch_names_set.add(c_name)
                except Exception:
                    pass

            done = len(ch_names_set)
            ch_list = sorted(list(ch_names_set))
            total = features._chapter_count(item) or done
            total = int(total) if total and total > 0 else done
            percent = None
            if total:
        ...
        first_url = rows[0].get("url") if rows else ""
        first_chs = out.get(first_url, {}).get("chapter_names", [])
        return {"ok": True, "status": out, "chapters": first_chs, "downloaded": first_chs}

    def rescan_library_books(self):
        """Rescan all library folders, discover new chapter archives, and sync counts."""
        res = library.scan_library_folders()
        books = self.get_library().get("books", [])
        return {"ok": True, "discovered": res.get("discovered", 0), "total": len(books)}

    def fix_missing_covers(self):
        """Fix, extract or download missing covers for all library books."""
        from ..covers import existing_cover, save_cover
        from ..metadata import search_mal
        lib = library.load_library()
        fixed = 0
        for key, entry in lib.items():
            cov = entry.get("cover")
            dir_path = entry.get("directory")
            if (not cov or not os.path.isfile(cov)) and dir_path and os.path.isdir(dir_path):
                found = existing_cover(dir_path)
                if found:
                    entry["cover"] = found
                    fixed += 1
                    continue
                title = entry.get("title") or os.path.basename(dir_path)
                try:
                    mal_res = search_mal(title, limit=1)
                    if mal_res and mal_res[0].get("cover"):
                        saved = save_cover(mal_res[0]["cover"], dir_path)
                        if saved:
                            entry["cover"] = saved
                            fixed += 1
                            continue
    ...
    def rebuild_library_metadata(self):
        """Rebuild both manga.json and ComicInfo.xml in each series directory."""
        from ..metadata import write_folder_metadata
        lib = library.load_library()
        rebuilt = 0
        for entry in lib.values():
            dir_path = entry.get("directory")
            if dir_path and os.path.isdir(dir_path):
                write_folder_metadata(dir_path, entry)
                rebuilt += 1
        return {"ok": True, "rebuilt": rebuilt}

    def clean_library_cache(self):
        """Clean orphan cover cache and partial download fragments."""
        from ..paths import data_dir
        cache_dir = os.path.join(data_dir(), "covers")
        cleaned = 0
        if os.path.isdir(cache_dir):
            for fname in os.listdir(cache_dir):
                if fname.endswith(".tmp") or fname.endswith(".part"):
                    try:
    ...
    def _spawn(self, entry):
        ...
        # Share the job id so mangasurf.progress tracks each concurrent job
