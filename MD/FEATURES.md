...
## 100% curl_cffi HTTP engine

Every request in Mangasurf goes through **curl_cffi** (`mangasurf/http.py`) —
the `requests` package has been removed entirely. This gives you:

- **Real browser impersonation** (`impersonate="chrome"` by default; switch with
  `MANGASURF_IMPERSONATE=safari|firefox|...`), so Cloudflare / Akamai bot walls
  are bypassed automatically via TLS/JA3+JA4 fingerprinting.
- **A fast async batch engine** (`http.fetch_many` / `http.download_many`).
  Chapter pages are streamed across a single libcurl multi handle, so a whole
  chapter arrives in roughly one page's latency.
- The requests-style exception names still work, so source plugins port with
  no changes.

## Per-chapter downloaded counts

Downloaded badges are accurate whether a series is stored as per-chapter files
or a single bundled CBZ. The search-result card and the manga detail view both
read the authoritative chapter list (via `downloaded_status` / `manga.json`),
so a bundle that holds 30 chapters shows "30 Downloaded" everywhere — not "1".

## Robustness engine (circuit breaker & caches)

`mangasurf/robust.py` keeps a dead site from dragging down every search and
browse. Per-target **circuit breaker** (closed / open / half-open states): once a
source fails `threshold` times, subsequent calls to it fail *instantly* until a
cooldown elapses, instead of sleeping through five exponential backoffs
(measured at 67.5s for one site before the fix, ~instant after). It also gives:

- **Retry with jitter** (`backoff_delay`) so concurrent workers don't retry in
  lockstep.
- **TTL caches** for browse and genre lists (`BROWSE_CACHE`, `GENRE_CACHE`).
- **Graceful degradation** (`call_safely`) — a failing enrichment returns its
  fallback rather than failing the whole run.
- **Health report** exposed to the UI: breaker state plus cache efficiency.

## Metadata, ComicInfo.xml and title decluttering

Every downloaded series folder is written with **dual metadata**:

- `manga.json` — chapters, local paths, page counts, reading progress.
- `ComicInfo.xml` — ComicRack / OPDS-compliant metadata (Title, Series, Writer,
  Genre, Rating, Year) generated by `metadata.generate_comic_info_xml`.

`metadata.declutter_title` strips scanlator tags, brackets and resolution
stamps ("Solo Leveling [Official] [1080p]" → "Solo Leveling") so filenames and
OPDS listings stay clean.

## Cover engine

`mangasurf/covers.py` extracts, groups and downloads series covers:
`scan` / `plan` to build a cover per series folder, `isolate` to split related
groups into their own folders, and `candidates` + `save_cover` to fetch a cover
from a chosen source (honouring `referer` for hotlink-protected CDNs).

## Local API

The reader server exposes a discovery-driven **Local API** (`mangasurf/localapi.py`)
at `GET /local/info` that lists every other endpoint, so clients can
self-discover: `books`, `reading`, `covers`, `sources`, `shelf_tree`, `stats`,
`paths`. Books on a locked shelf are withheld from every endpoint.

## 3D depth cover carousel

The reader's library uses an immersive 7-card **3D depth carousel** with
perspective, per-title reading-progress meters, custom descriptions and
click-to-read.
...
| Mewhen18 | mewhen18.com | 18+ (successor to HentaiAkane) |
