...
:mod:`readerm.sources.madara` scrapes it; this module turns the whole set of
...
-----------------------
* :mod:`readerm.sources.madara` -- the scraping engine. Not a source, never in
  the UI.
* :mod:`readerm.sources.madarascans` -- *Madara Scans*, an unrelated
  scanlation site that does not even run this theme.

Members
-------
Each entry is a full :class:`~readerm.sources.madara.MadaraSource` subclass,
...
per-site circuit breaker in :mod:`readerm.robust` still applies to each member
individually.
